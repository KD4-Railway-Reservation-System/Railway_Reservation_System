...
export default function TrainCard({ train, stations = [] }) {
  ...
  const getSourceStationDisplay = () => {
    ...
    const st = stations.find((s) => String(s.id || s.stationId) === String(train.sourceStationId));
    return st ? `${st.stationName} (${st.stationCode})` : `Station #${train.sourceStationId}`;
  };
  ...
  const getDestStationDisplay = () => {
    ...
    const st = stations.find((s) => String(s.id || s.stationId) === String(train.destinationStationId));
    return st ? `${st.stationName} (${st.stationCode})` : `Station #${train.destinationStationId}`;
  };

  const isAvailable = (train.availableSeats ?? train.totalSeats ?? 100) > 0;
  const availCount = train.availableSeats ?? Math.floor((train.totalSeats || 300) * 0.85);
    ...
    <div className="glass-card glass-card-hover rounded-2xl p-6 relative overflow-hidden group border border-slate-800">
      ...
      <div className="grid grid-cols-3 items-center py-6 my-2 text-center sm:text-left">
        <div>
          ...
          <p className="text-base font-bold text-white mt-1">{getSourceStationDisplay()}</p>
        ...
        <div className="flex flex-col items-center justify-center">
          <span className="text-xs text-slate-500 font-medium mb-1">Superfast Express</span>
        ...
        <div className="text-center sm:text-right">
          <p className="text-xs font-medium text-slate-400 uppercase tracking-wider">To</p>
          <p className="text-base font-bold text-white mt-1">{getDestStationDisplay()}</p>
      ...
      <div className="pt-4 border-t border-slate-800/80 flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div className="flex items-center space-x-4 w-full sm:w-auto justify-between sm:justify-end">
          <div>
            ...
            <span className="text-xl font-bold text-white">₹{train.fare || 1450}</span>
          </div>
